...
def finalSpiter(text):
    rows = []
    thirdPattern = r'^(?=\([\dZTBg\. ]{1,5}\))|^(?=\d{1,2}\.?\d{0,2}\.?\d?\.?\d?\)[^\n])|^(?=[1-3]{1,2}\.?\d\.?\d\.?\d? [เค])|^(?=ค่าใช้จ่ายในการบำรุงรักษาเครื่องปรับอากาศ)|^(?=- เงินรายได้)|^(?=2 ค่าสาธารณูปโภค)|^(?=1 ค่าใช้สอย)|^(?=โครงการสร้างโอกาสในการพัฒนาสมรรถนะของผู้ประกอบอาชี)'
    yearPattern = r'^(?=วงเงินทั้งสิ้?น)|^(?=.*ตั้งงบประมา?ณ?)|^(?=.*ผูกพันงบประมาณ)|^(?=เงินงบประมาณ)|^(?=เงินนอกงบประมาณ)'
